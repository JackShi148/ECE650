...
int main(int args, char * argv[]) {
    ...
    int id, player_num;
    recv(connect_rm_sockfd, &id, sizeof(id), MSG_WAITALL);
    recv(connect_rm_sockfd, &player_num, sizeof(player_num), MSG_WAITALL);
    ...
    send(connect_rm_sockfd, &as_server_port, sizeof(as_server_port), 0);

    // init as client to its right neighbor
    char rneighbor_addr[100];
    int rneighbor_port_int;
    recv(connect_rm_sockfd, &rneighbor_addr, sizeof(rneighbor_addr), MSG_WAITALL);
    recv(connect_rm_sockfd, &rneighbor_port_int, sizeof(rneighbor_port_int), MSG_WAITALL);
    ...
    while(true) {
        ...
        if(potato.hops_num == 0) {
            cout << "I'm it" << endl;
            send(connect_rm_sockfd, &potato, sizeof(potato), 0);
        }
        else {
            ...
            if(rand_id == 0) {
                ...
                send(connect_lneighbor_sockfd, &potato, sizeof(potato), 0);
            }
            // send to right neighbor
            else {
                cout << "Sending potato to " << right_neighbor_id << endl;
                send(connect_rneighbor_sockfd, &potato, sizeof(potato), 0);
            }
        }
    }
