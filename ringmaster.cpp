...
int main(int args, char * argv[]) {
    ...
    if(hops_num != 0) {
        ...
        send(rm_connection_player_fds[rand_id], &potato, sizeof(potato), 0);
    ...
    if(potato.hops_num != 0) {
        cerr << "Error: potato's num_hops are not 0" << endl;
        return EXIT_FAILURE;
    }
    // notify players of closing the game
    for(int i = 0; i < rm_connection_player_fds.size(); i++) {
        send(rm_connection_player_fds[i], &potato, sizeof(potato), 0);
    }
